...
export function haversineDistance(lat1, lon1, lat2, lon2) {
  ...
  if (lat1 < -90 || lat1 > 90 || lat2 < -90 || lat2 > 90) return Number.POSITIVE_INFINITY;
  if (lon1 < -180 || lon1 > 180 || lon2 < -180 || lon2 > 180) return Number.POSITIVE_INFINITY;
...
export function buildVector(item, maps = {}) {
  const { categoryMap = new Map(), genderMap = new Map() } = maps;
